...
class MyVol:
	...
	def __init__(self,coin_pair,tapi_nonce=str(int(time.time()))):
		...
		s = dataAtual.strftime('%d/%m/%Y')
		#dataAtual = '28/07/2020'
		from_timestamp = time.mktime(datetime.datetime.strptime(s, "%d/%m/%Y").timetuple())
	...
	def getMyVol(self):
		...
		while True:
			try:
				quantidade = quantidade + float(self.getOrdersExecutedQuantity(cont))				
			except:
				return quantidade
			cont=cont+1
		return quantidade
